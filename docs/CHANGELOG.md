...
### Output T — Leveling (client-side, local player only)

- Level-up folded into the existing `add_exp` RPC path — **zero new synced property, zero new RPC**, so no replication-schema mismatch and no forced redeploy (dodges the CLAUDE.md gotcha the same way EXP-on-kill did).
- Curve kept dead simple: level up while **total EXP ≥ `level * 30`** (cumulative thresholds — EXP never resets). Creatures give 10 EXP, so 3 kills → Lv 2, 3 more (60 total) → Lv 3, etc. Easy to feel in a playtest.
- Groundwork: the old `MAX_HP` **const** became a `var max_hp` (leveling mutates it), plus new `var level: int = 1`, a `LevelLabel` (`Label`) added under `$HUD`, and `@onready var level_label`. The HUD-init block (authority-gated) sets `level_label.text = "Lv %d" % level` alongside the HP-bar init.
- On each level-up: `level += 1`, `max_hp += 20`, `hp = max_hp` (heal to full), then `hp_bar.max_value` set **before** `hp_bar.value` — a `ProgressBar` clamps `value` to the current `max_value`, so bumping value first would silently swallow the new HP. `level_label` updated last.
- Used a `while` loop, not an `if`: one kill (10 EXP) can't cross a 30-EXP gap, so `if` would suffice today — but `while` is correct-by-construction if a future grant (quest/boss reward, or a flatter curve) ever spans multiple levels. Costs nothing, removes a baked-in assumption.
- The `exp` variable was renamed to `experience` throughout to stop it shadowing GDScript's built-in `exp()` function. The **RPC name stayed `add_exp`**, so `creature.gd`'s `player.add_exp.rpc_id(killer_id, …)` still resolves — no protocol change.
- **Accepted caveats (fine for a friend playtest):** other players don't see your level (no synced level number — parking-lot), and the buffed max HP is client-computed, so it's spoofable. A synced level can be one deliberate redeploy later if it ever matters.
- Client-side only → **no forced redeploy**, but `player.gd` also runs on the server, so sync builds (redeploy + restart clients) at the next deploy for hygiene.
